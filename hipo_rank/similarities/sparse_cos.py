...
class SparseCosSimilarity:
    ...
    def get_similarities(self, embeds: Embeddings):
        ...
        sparse_sent_to_sect = []
        # sparse cross section sentence embedding.
        for sent_embeds in embeds.sentence:
            pair_indices = []
            num_sent = len(sent_embeds.embeddings)
            
            sparse_rep = []
            for sent_embed2 in embeds.sentence:
            # for for sent_embed2 in embeds.sentence:
                a = np.arange(len(sent_embed2.embeddings))
                np.random.shuffle(a)
                # print(a)
                sample_index = a[:max(1,int(0.5*len(a)))]
                # print(sample_index)
                
                # print(sent_embed2.embeddings[sample_index])
                score = np.mean(sent_embed2.embeddings[sample_index], axis=0)
                # print(score)
                
                sparse_rep.append(score)
            ...
            for i in range(num_sent):
                for j in range(num_sect):
                    pair_indices += [(i,j)]
            sparse_rep = np.stack(sparse_rep)       
            pair_indices_i = [x[0] for x in pair_indices]
            pair_indices_j = [x[1] for x in pair_indices]      
            embed1 = sent_embeds.embeddings[pair_indices_i] 
            embed2 = sparse_rep[pair_indices_j]
            similarities = self._compute_similarities(embed1, embed2)
            
            id = sent_embeds.id
            directions = ["undirected" for _ in pair_indices]
            sparse_sent_to_sect += [SentenceSimilarities(id, similarities, pair_indices, directions)]
                
                
                
        
        return Similarities(sent_to_sent, sect_to_sect, sent_to_sect, sparse_sent_to_sect)
