...
class AddScorer:
    ...
    def __init__(self,
        ...
        self._beta = _beta
        print("Scorer Truncation", self.truncate)
    ...
    def get_scores(self, similarities: Similarities) -> Scores:
        ...
        for sect_index, sent_to_sent in enumerate(similarities.sent_to_sent):
            ...
            # truncated score. /beta * (max - min)
            # print(sims)
            if sims != []:
                threshold = np.percentile(np.asarray(sims), 0)
            else:
                threshold = 0
            for ((i,j), dir, sim) in zip(pids, dirs, sims):
                if self.truncate == True:
                    sim = max(sim - threshold, 0)
