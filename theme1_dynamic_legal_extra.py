...
from hipo_rank.similarities.bonus_cos import BonusCosSimilarity
...
from hipo_rank.scorers.dynamic_sent import DynamicSentScorer
from hipo_rank.summarizers.default import DynamicSentSummarizer, DefaultSummarizer
...
DATASETS = [
            ("pubmed_test", PubmedDataset, {"file_path": "data/legal_1049_pubmed_format_no_header.txt"}),
            ]

DATASETS = [
                    ("pubmed_test", PubmedDataset, {"file_path": "data/different_theme/legal_1049_pubmed_format_label1_no_header.txt"}),
                                ]
EMBEDDERS = [
...
SCORERS = [
        ("add_f=0.0_b=1.0_s=0.5", AddScorer, {"section_weight": 0.5, 'sparse':False}),
]


Summarizer = DynamicSentSummarizer(start_ratio=0.5)

experiment_time = int(time.time())
results_path = Path(f"results/exp3_1049_dynamic_sent_theme1")
sparse = False
        ...
        for similarity_id, similarity, similarity_args in SIMILARITIES:
            ...
            for direction_id, direction, direction_args in DIRECTIONS:
                ...
                for scorer_id, scorer, scorer_args in SCORERS:
                    ...
                    dScorer = DynamicSentScorer(**scorer_args)
